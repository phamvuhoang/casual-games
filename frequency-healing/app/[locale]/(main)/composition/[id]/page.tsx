...
function extractStorageObjectPath(publicUrl: string | null, bucket: string) {
  ...
  try {
    ...
    const markers = [`/storage/v1/object/public/${bucket}/`, `/object/public/${bucket}/`, `/${bucket}/`];
...
export default function CompositionPage() {
  ...
  const tStatus = useTranslations('composition.status');
  ...
  const handleLike = async () => {
    ...
    setStatus(null);
    if (!userId) {
      setStatus(tStatus('signInToLike'));
      setIsLiking(false);
      return;
    }
  ...
  const handleSubmitComment = async () => {
    ...
    if (!trimmed) {
      setCommentStatus(tStatus('writeComment'));
      return;
    }

    if (!userId) {
      setCommentStatus(tStatus('signInToComment'));
      return;
    }
    ...
    try {
      ...
      console.error(error);
      setCommentStatus(tStatus('verifyProfile'));
      setIsPostingComment(false);
      return;
    }
  ...
  const handlePlay = async () => {
    ...
    setStatus(null);
    setComposition((prev) => (prev ? { ...prev, play_count: (prev.play_count ?? 0) + 1 } : prev));
    ...
    if (error) {
      console.warn(error);
      setStatus(error.message);
      setComposition((prev) => (prev ? { ...prev, play_count: Math.max(0, (prev.play_count ?? 1) - 1) } : prev));
  ...
  const handleDeleteComposition = async () => {
    ...
    if (!userId) {
      setDeleteStatus(tStatus('signInToDelete'));
      return;
    }

    if (!composition.user_id || composition.user_id !== userId) {
      setDeleteStatus(tStatus('onlyCreatorDelete'));
      return;
    }

    const confirmed = typeof window === 'undefined' ? false : window.confirm(tStatus('deleteConfirm'));
    if (!confirmed) {
      return;
    }
    ...
    try {
      ...
      for (const target of mediaTargets) {
        ...
        if (!parsedPath) {
          throw new Error(tStatus('deleteParsePath', { label: target.label }));
        }
      ...
      for (const [bucket, pathSet] of pathsByBucket.entries()) {
        ...
        if (error) {
          throw new Error(tStatus('deleteStorageFailed', { bucket, message: error.message }));
        }
      }

      const { error: deleteError } = await supabase.from('compositions').delete().eq('id', composition.id).eq('user_id', userId);
      ...
      console.error(error);
      const message = error && typeof error === 'object' && 'message' in error ? String(error.message) : tStatus('deleteFailed');
  ...
  if (!composition) {
    return <p className="text-sm text-ink/70">{status ?? t('loading')}</p>;
  }
    ...
    return (
      <div className="mx-auto flex w-full max-w-3xl flex-col gap-4 p-4">
        ...
        <div className="rounded-3xl border border-ink/10 bg-white/80 p-4 text-sm text-ink/70">
          {composition.description?.trim() || t('embedFallbackDescription')}
        </div>
      </div>
    );
  }
    ...
    <div className="mx-auto flex w-full max-w-5xl flex-col gap-8">
      ...
      <div className="grid gap-6 md:grid-cols-[1.2fr_0.8fr]">
        <Card className="glass-panel">
          ...
          {composition.description ? (
            ...
            <p className="mt-3 text-sm text-ink/60">{t('noDescription')}</p>
          )}
          <div className="mt-4 flex flex-wrap gap-4 text-sm text-ink/60">
            <span>{t('waveform')}: {composition.waveform ?? 'sine'}</span>
            <span>{t('duration')}: {composition.duration ?? 0}s</span>
            <span>{t('ambient')}: {composition.ambient_sound ?? 'none'}</span>
          </div>
        </Card>
        <Card className="glass-panel">
          <h2 className="text-lg font-semibold">{t('communityEnergy')}</h2>
          <p className="mt-3 text-sm text-ink/70">
            {t('likesPlays', { likes: composition.like_count ?? 0, plays: composition.play_count ?? 0 })}
          </p>
          <Button onClick={handleLike} disabled={isLiking} className="mt-4">
            {isLiking ? t('updating') : hasLiked ? t('removeAppreciation') : t('sendAppreciation')}
          </Button>
      ...
      {isOwner ? (
        <Card className="glass-panel border border-rose-200/70 bg-rose-50/70">
          <h2 className="text-lg font-semibold text-rose-900">{t('creatorTools')}</h2>
          <p className="mt-3 text-sm text-rose-900/80">{t('creatorDeleteDescription')}</p>
          <Button
            variant="outline"
          ...
          >
            {isDeletingComposition ? t('deleting') : t('deleteComposition')}
          </Button>
      ...
      <Card className="glass-panel">
        <h2 className="text-lg font-semibold">{t('saveToCollection')}</h2>
        {userId ? (
                ...
                return (
                  ...
                  >
                    <div>
                      ...
                      {collection.description ? <p className="text-xs text-ink/60">{collection.description}</p> : null}
                    </div>
                    <Button
                      size="sm"
          ...
          <div className="mt-4 rounded-2xl border border-ink/10 bg-white/80 p-4">
            <p className="text-sm text-ink/70">{t('signInToSaveDescription')}</p>
            <div className="mt-3">
              <Button asChild size="sm">
                <Link href={`/login?redirectTo=/composition/${composition.id}`}>{t('signInToSave')}</Link>
              </Button>
            </div>
          </div>
        )}
      ...
      <Card className="glass-panel">
        <h2 className="text-lg font-semibold">{t('communityReflections')}</h2>
        {userId ? (
          <div className="mt-4 space-y-3">
            <textarea
              value={commentInput}
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <span className="text-xs text-ink/50">{t('commentHint')}</span>
            </div>
          </div>
        ) : (
          <div className="mt-4 rounded-2xl border border-ink/10 bg-white/80 p-4">
            <p className="text-sm text-ink/70">{t('signInToReflectDescription')}</p>
            <div className="mt-3">
              <Button asChild size="sm">
                <Link href={`/login?redirectTo=/composition/${composition.id}`}>{t('signInToComment')}</Link>
              </Button>
            </div>
          </div>
        )}
        {commentStatus ? <p className="mt-3 text-sm text-rose-500">{commentStatus}</p> : null}
        {isLoadingComments ? <p className="mt-4 text-sm text-ink/60">{t('loadingReflections')}</p> : null}
        {!isLoadingComments && comments.length === 0 ? <p className="mt-4 text-sm text-ink/60">{t('noReflections')}</p> : null}
        <div className="mt-4 grid gap-4">
          {comments.map((comment) => {
            const displayName = comment.author?.display_name ?? comment.author?.username ?? t('anonymousListener');
