...
function formatHz(values: number[], emptyLabel: string) {
  if (values.length === 0) {
    return emptyLabel;
  }
...
function formatKeywords(values: string[], emptyLabel: string) {
  if (values.length === 0) {
    return emptyLabel;
  }
  return values.join(', ');
...
export default function ProfilePage() {
  ...
  const tHarmonicPresets = useTranslations('create.frequencyCreator.harmonicPresets');
  ...
  const creatorPath = '/create';

  const formatDateTimeLabel = (value: string) => formatDateTime(value, locale);
  const formatHzLabel = (values: number[]) => formatHz(values, noneLabel);
  const formatKeywordsLabel = (values: string[]) => formatKeywords(values, noneLabel);
  const formatBreathModeLabel = (mode: string) =>
    mode === 'microphone' ? t('breath.modeMicrophone') : t('breath.modeManual');
  const formatHarmonicPresetLabel = (presetId: string) => {
    if (presetId === 'unknown') return t('unknown');
    if (presetId === 'chakra_ladder') return tHarmonicPresets('chakra_ladder.name');
    if (presetId === 'heart_bridge') return tHarmonicPresets('heart_bridge.name');
    if (presetId === 'release_reset') return tHarmonicPresets('release_reset.name');
    if (presetId === 'clarity_focus') return tHarmonicPresets('clarity_focus.name');
    if (presetId === 'spiral_overtone') return tHarmonicPresets('spiral_overtone.name');
    if (presetId === 'earth_sky') return tHarmonicPresets('earth_sky.name');
    return presetId || t('unknown');
  };
  ...
  const handleCreateCollection = async () => {
    if (!profile || !viewerId || !isOwner) {
      setCollectionStatus(t('status.onlyOwnerCanCreateCollections'));
      return;
    }

    const trimmedName = collectionName.trim();
    if (!trimmedName) {
      setCollectionStatus(t('status.collectionNameRequired'));
      return;
    }
  ...
  const handleSaveProfile = async () => {
    ...
    setProfile(data);
    setProfileStatus(t('status.profileUpdated'));
    setIsSavingProfile(false);
  };

  if (!profile) {
    return <p className="text-sm text-ink/70">{status ?? t('status.loadingProfile')}</p>;
  }

  return (
    <div className="mx-auto flex w-full max-w-5xl flex-col gap-8">
      <div className="flex flex-col gap-4">
        <p className="text-xs uppercase tracking-[0.35em] text-ink/60">{t('profileLabel')}</p>
      ...
      {isOwner ? (
        <Card className="glass-panel">
          ...
          <div className="mt-4 grid gap-3">
            <label className="text-xs uppercase tracking-[0.3em] text-ink/60">{t('editProfile.displayName')}</label>
            <input
              value={profileDisplayName}
              onChange={(event) => setProfileDisplayName(event.target.value)}
              className="rounded-2xl border border-ink/10 bg-white/90 px-4 py-3 text-sm"
            />
            <label className="text-xs uppercase tracking-[0.3em] text-ink/60">{t('editProfile.avatarUrl')}</label>
            <input
              value={profileAvatarUrl}
              onChange={(event) => setProfileAvatarUrl(event.target.value)}
              className="rounded-2xl border border-ink/10 bg-white/90 px-4 py-3 text-sm"
            />
            <label className="text-xs uppercase tracking-[0.3em] text-ink/60">{t('editProfile.bio')}</label>
            <textarea
              value={profileBio}
            ...
            <Button size="sm" onClick={handleSaveProfile} disabled={isSavingProfile}>
              {isSavingProfile ? t('editProfile.saving') : t('editProfile.save')}
            </Button>
      ...
      <Card className="glass-panel">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="text-sm text-ink/60">{t('collections.subtitle')}</p>
          </div>
          <span className="text-xs uppercase tracking-[0.2em] text-ink/50">
            {t('collections.totalCount', { count: collections.length })}
          </span>
        </div>

        {isOwner ? (
          <div className="mt-4 grid gap-3 rounded-2xl border border-ink/10 bg-white/70 p-4">
            <label className="text-xs uppercase tracking-[0.3em] text-ink/60">{t('collections.createLabel')}</label>
            <input
            ...
            <textarea
              ...
              placeholder={t('collections.descriptionOptionalPlaceholder')}
            ...
            <Button size="sm" onClick={handleCreateCollection} disabled={isCreatingCollection}>
              {isCreatingCollection ? t('collections.creating') : t('collections.create')}
            </Button>
            ...
            <Card key={collection.id} className="glass-panel border border-ink/10">
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs text-ink/60">
                  {t('collections.sessionsCount', { count: collectionCounts[collection.id] ?? 0 })}
                </span>
              </div>
              ...
              <p className="mt-3 text-xs text-ink/50">
                {collection.is_public ? t('collections.publicPlaylist') : t('collections.privatePlaylist')}
              </p>
            </Card>
          ))}
      ...
      {isOwner ? (
        <Card className="glass-panel">
          <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
            <div>
              <h2 className="text-lg font-semibold">{t('voiceHistory.title')}</h2>
              <p className="text-sm text-ink/60">
                {t('voiceHistory.subtitle')}
              </p>
            </div>
            <span className="text-xs uppercase tracking-[0.2em] text-ink/50">
              {t('voiceHistory.capturesCount', { count: voiceHistory.length })}
            </span>
          </div>
          ...
          {voiceHistory.length === 0 ? (
            <div className="mt-4 rounded-2xl border border-ink/10 bg-white/70 p-4 text-sm text-ink/60">
              <p>{t('voiceHistory.emptyState')}</p>
              <div className="mt-3">
                <Button asChild size="sm" variant="outline">
                  <Link href={creatorPath}>{t('openCreator')}</Link>
                </Button>
              </div>
            </div>
          ) : (
            <>
              <div className="mt-4 grid gap-3 md:grid-cols-3">
                <div className="rounded-2xl border border-ink/10 bg-white/80 p-3">
                  <p className="text-xs uppercase tracking-[0.2em] text-ink/55">{t('voiceHistory.latestConfidence')}</p>
                  <p className="mt-1 text-lg font-semibold text-ink/85">
                    {latestVoiceProfile ? formatPercent(latestVoiceProfile.confidence) : naLabel}
                  </p>
                </div>
                <div className="rounded-2xl border border-ink/10 bg-white/80 p-3">
                  <p className="text-xs uppercase tracking-[0.2em] text-ink/55">{t('voiceHistory.captureTrend')}</p>
                  <p className="mt-1 text-lg font-semibold text-ink/85">
                    {latestConfidenceDelta !== null ? formatSignedPercent(latestConfidenceDelta) : naLabel}
                  </p>
                  <p className="text-xs text-ink/55">{t('voiceHistory.comparedToPrevious')}</p>
                </div>
                <div className="rounded-2xl border border-ink/10 bg-white/80 p-3">
                  <p className="text-xs uppercase tracking-[0.2em] text-ink/55">{t('voiceHistory.lastCaptured')}</p>
                  <p className="mt-1 text-sm font-semibold text-ink/85">
                    {latestVoiceProfile ? formatDateTimeLabel(latestVoiceProfile.capturedAt) : naLabel}
                  </p>
                </div>
              </div>

              <div className="mt-4 rounded-2xl border border-ink/10 bg-white/75 p-4">
                <h3 className="text-sm font-semibold uppercase tracking-[0.2em] text-ink/60">{t('voiceHistory.compareTrends')}</h3>
                <div className="mt-3 grid gap-3 md:grid-cols-2">
                  <label className="text-sm text-ink/70">
                    <span className="mb-1 block text-xs uppercase tracking-[0.2em] text-ink/55">{t('voiceHistory.current')}</span>
                    <select
                    ...
                    >
                      {voiceHistory.map((entry) => (
                        <option key={`current-${entry.id}`} value={entry.id}>
                          {formatDateTimeLabel(entry.capturedAt)} ({formatPercent(entry.confidence)})
                        </option>
                      ))}
                    </select>
                  </label>
                  <label className="text-sm text-ink/70">
                    <span className="mb-1 block text-xs uppercase tracking-[0.2em] text-ink/55">{t('voiceHistory.baseline')}</span>
                    <select
                ...
                {canCompare ? (
                  <div className="mt-4 grid gap-3 md:grid-cols-2">
                    <div className="rounded-xl border border-ink/10 bg-white px-3 py-2 text-sm text-ink/70">
                      <p className="text-xs uppercase tracking-[0.2em] text-ink/55">{t('voiceHistory.confidenceDelta')}</p>
                      <p className="mt-1 font-semibold text-ink/85">
                        {compareConfidenceDelta !== null ? formatSignedPercent(compareConfidenceDelta) : naLabel}
                      </p>
                    </div>
                    <div className="rounded-xl border border-ink/10 bg-white px-3 py-2 text-sm text-ink/70">
                      <p className="text-xs uppercase tracking-[0.2em] text-ink/55">{t('voiceHistory.bandEnergyDelta')}</p>
                      <p className="mt-1 text-xs text-ink/70">
                        {t('voiceHistory.low')}: {compareLowDelta !== null ? formatSignedPercent(compareLowDelta) : naLabel} | {t('voiceHistory.mid')}:{' '}
                        {compareMidDelta !== null ? formatSignedPercent(compareMidDelta) : naLabel}
                      </p>
                      <p className="text-xs text-ink/70">
                        {t('voiceHistory.upperMid')}: {compareUpperMidDelta !== null ? formatSignedPercent(compareUpperMidDelta) : naLabel} | {t('voiceHistory.high')}:{' '}
                        {compareHighDelta !== null ? formatSignedPercent(compareHighDelta) : naLabel}
                      </p>
                    </div>
                    <div className="rounded-xl border border-ink/10 bg-white px-3 py-2 text-sm text-ink/70">
                      <p className="text-xs uppercase tracking-[0.2em] text-ink/55">{t('voiceHistory.currentDominantTones')}</p>
                      <p className="mt-1 text-xs text-ink/75">
                        {compareCurrent ? formatHzLabel(compareCurrent.dominantFrequencies) : naLabel}
                      </p>
                    </div>
                    <div className="rounded-xl border border-ink/10 bg-white px-3 py-2 text-sm text-ink/70">
                      <p className="text-xs uppercase tracking-[0.2em] text-ink/55">{t('voiceHistory.currentRecommendations')}</p>
                      <p className="mt-1 text-xs text-ink/75">
                        {compareCurrent ? formatHzLabel(compareCurrent.recommendedFrequencies) : naLabel}
                      </p>
                    </div>
                  </div>
                ) : (
                  <p className="mt-3 text-xs text-ink/60">{t('voiceHistory.selectTwoCaptures')}</p>
                )}
              </div>
                  ...
                  <div key={entry.id} className="rounded-2xl border border-ink/10 bg-white/80 p-3">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <p className="text-sm font-semibold text-ink/85">
                        {t('voiceHistory.captureNumber', { number: voiceHistory.length - index })} · {formatDateTimeLabel(entry.capturedAt)}
                      </p>
                      <span className="text-xs text-ink/60">
                        {t('voiceHistory.confidenceLabel')} {formatPercent(entry.confidence)}
                      </span>
                    </div>
                    <p className="mt-1 text-xs text-ink/60">
                      {t('voiceHistory.dominantLabel')}: {formatHzLabel(entry.dominantFrequencies)} | {t('voiceHistory.recommendedLabel')}: {formatHzLabel(entry.recommendedFrequencies)}
                    </p>
                    <p className="mt-1 text-xs text-ink/55">
                      {t('voiceHistory.low')} {formatPercent(entry.bandEnergy.low)} · {t('voiceHistory.mid')} {formatPercent(entry.bandEnergy.mid)} · {t('voiceHistory.upperMid')}{' '}
                      {formatPercent(entry.bandEnergy.upperMid)} · {t('voiceHistory.high')} {formatPercent(entry.bandEnergy.high)}
                    </p>
                    <p className="mt-1 text-xs text-ink/50">
                      {t('voiceHistory.captureDuration', { seconds: Math.round(entry.captureDurationMs / 1000) })} · {t('voiceHistory.analysisDuration', { ms: entry.analysisDurationMs })}
                    </p>
                  </div>
                ))}
              </div>
      ...
      {isOwner ? (
        <Card className="glass-panel">
          <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
            <div>
              <h2 className="text-lg font-semibold">{t('breath.title')}</h2>
              <p className="text-sm text-ink/60">
                {t('breath.subtitle')}
              </p>
            </div>
            <span className="text-xs uppercase tracking-[0.2em] text-ink/50">
              {t('breath.sessionsCount', { count: breathHistory.length })}
            </span>
          </div>
          ...
          {breathHistory.length === 0 ? (
            ...
            <>
              <div className="mt-4 grid gap-3 md:grid-cols-4">
                <div className="rounded-2xl border border-ink/10 bg-white/80 p-3">
                  <p className="text-xs uppercase tracking-[0.2em] text-ink/55">{t('breath.latestCoherence')}</p>
                  <p className="mt-1 text-sm font-semibold text-ink/85">
                    {latestBreathSession ? formatPercent(latestBreathSession.coherenceScore) : naLabel}
                  </p>
                </div>
                <div className="rounded-2xl border border-ink/10 bg-white/80 p-3">
                  <p className="text-xs uppercase tracking-[0.2em] text-ink/55">{t('breath.coherenceTrend')}</p>
                  <p className="mt-1 text-sm font-semibold text-ink/85">
                    {breathCoherenceDelta !== null ? formatSignedPercent(breathCoherenceDelta) : naLabel}
                  </p>
                </div>
                <div className="rounded-2xl border border-ink/10 bg-white/80 p-3">
                  <p className="text-xs uppercase tracking-[0.2em] text-ink/55">{t('breath.avgBreathRate')}</p>
                  <p className="mt-1 text-sm font-semibold text-ink/85">
                    {breathAverageRate !== null ? t('breath.rateBpm', { bpm: breathAverageRate.toFixed(1) }) : naLabel}
                  </p>
                </div>
                <div className="rounded-2xl border border-ink/10 bg-white/80 p-3">
                  <p className="text-xs uppercase tracking-[0.2em] text-ink/55">{t('breath.avgCoherence')}</p>
                  <p className="mt-1 text-sm font-semibold text-ink/85">
                    {breathAverageCoherence !== null ? formatPercent(breathAverageCoherence) : naLabel}
                  </p>
                </div>
              </div>
              ...
              <div className="mt-4 grid gap-3">
                {breathHistory.map((entry, index) => (
                  <div key={entry.id} className="rounded-2xl border border-ink/10 bg-white/80 p-3">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <p className="text-sm font-semibold text-ink/85">
                        {t('breath.sessionNumber', { number: breathHistory.length - index })} · {formatDateTimeLabel(entry.createdAt)}
                      </p>
                      <span className="text-xs text-ink/60">
                        {formatBreathModeLabel(entry.mode)} · {t('breath.coherenceLabel')} {formatPercent(entry.coherenceScore)}
                      </span>
                    </div>
                    <p className="mt-1 text-xs text-ink/60">
                      {t('breath.rateLabel')} {entry.averageBreathBpm.toFixed(1)} bpm ({t('breath.targetLabel')} {entry.targetBpm.toFixed(1)} bpm) · {t('breath.peakLabel')}{' '}
                      {formatPercent(entry.peakCoherenceScore)}
                    </p>
                    <p className="mt-1 text-xs text-ink/55">
                      {t('breath.timeInCoherenceLabel')} {formatPercent(entry.timeInCoherencePct)} · {t('breath.inhaleRatioLabel')}{' '}
                      {Math.round(entry.inhaleRatio * 100)}%
                    </p>
                    <p className="mt-1 text-xs text-ink/50">
                      {t('breath.sensitivityLabel')} {Math.round(entry.sensitivity * 100)}% · {t('breath.samplesCount', { count: entry.sampleCount })}
                    </p>
                  </div>
                ))}
              </div>
      ...
      {isOwner ? (
        <Card className="glass-panel">
          <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
            <div>
              <h2 className="text-lg font-semibold">{t('harmonic.title')}</h2>
              <p className="text-sm text-ink/60">
                {t('harmonic.subtitle')}
              </p>
            </div>
            <span className="text-xs uppercase tracking-[0.2em] text-ink/50">
              {t('harmonic.sessionsCount', { count: harmonicHistory.length })}
            </span>
          </div>
          ...
          {harmonicHistory.length === 0 ? (
            ...
            <>
              <div className="mt-4 grid gap-3 md:grid-cols-4">
                <div className="rounded-2xl border border-ink/10 bg-white/80 p-3">
                  <p className="text-xs uppercase tracking-[0.2em] text-ink/55">{t('harmonic.latestPreset')}</p>
                  <p className="mt-1 text-sm font-semibold text-ink/85">
                    {latestHarmonicSession ? formatHarmonicPresetLabel(latestHarmonicSession.presetId) : naLabel}
                  </p>
                </div>
                <div className="rounded-2xl border border-ink/10 bg-white/80 p-3">
                  <p className="text-xs uppercase tracking-[0.2em] text-ink/55">{t('harmonic.avgIntensity')}</p>
                  <p className="mt-1 text-sm font-semibold text-ink/85">
                    {harmonicAverageIntensity !== null ? formatPercent(harmonicAverageIntensity) : naLabel}
                  </p>
                </div>
                <div className="rounded-2xl border border-ink/10 bg-white/80 p-3">
                  <p className="text-xs uppercase tracking-[0.2em] text-ink/55">{t('harmonic.intensityTrend')}</p>
                  <p className="mt-1 text-sm font-semibold text-ink/85">
                    {harmonicIntensityDelta !== null ? formatSignedPercent(harmonicIntensityDelta) : naLabel}
                  </p>
                </div>
                <div className="rounded-2xl border border-ink/10 bg-white/80 p-3">
                  <p className="text-xs uppercase tracking-[0.2em] text-ink/55">{t('harmonic.mostUsedPreset')}</p>
                  <p className="mt-1 text-sm font-semibold text-ink/85">
                    {harmonicTopPreset ? formatHarmonicPresetLabel(harmonicTopPreset) : naLabel}
                  </p>
                </div>
              </div>
              ...
              <div className="mt-4 grid gap-3">
                {harmonicHistory.map((entry, index) => (
                  <div key={entry.id} className="rounded-2xl border border-ink/10 bg-white/80 p-3">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <p className="text-sm font-semibold text-ink/85">
                        {t('harmonic.sessionNumber', { number: harmonicHistory.length - index })} · {formatDateTimeLabel(entry.createdAt)}
                      </p>
                      <span className="text-xs text-ink/60">
                        {t('harmonic.intensityLabel')} {formatPercent(entry.intensity)} · {t('harmonic.motionLabel')} {formatPercent(entry.motionSpeed)}
                      </span>
                    </div>
                    <p className="mt-1 text-xs text-ink/60">
                      {t('harmonic.presetLabel')} {formatHarmonicPresetLabel(entry.presetId)} · {t('harmonic.layersLabel')} {formatHzLabel(entry.layerFrequencies)}
                    </p>
                    <p className="mt-1 text-xs text-ink/55">
                      {t('harmonic.interferenceLabel')}:{' '}
                    ...
                    <p className="mt-1 text-xs text-ink/50">
                      {entry.includeInterference ? t('harmonic.interferenceOn') : t('harmonic.interferenceOff')} ·{' '}
                      {entry.spatialMotionEnabled ? t('harmonic.spatialMotionOn') : t('harmonic.spatialMotionOff')}
                    </p>
                  </div>
                ))}
              </div>
      ...
      {isOwner ? (
        <Card className="glass-panel">
          <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
            <div>
              <h2 className="text-lg font-semibold">{t('intention.title')}</h2>
              <p className="text-sm text-ink/60">
                {t('intention.subtitle')}
              </p>
            </div>
            <span className="text-xs uppercase tracking-[0.2em] text-ink/50">
              {t('intention.imprintsCount', { count: intentionHistory.length })}
            </span>
          </div>
          ...
          {intentionHistory.length === 0 ? (
            ...
            <>
              <div className="mt-4 grid gap-3 md:grid-cols-4">
                <div className="rounded-2xl border border-ink/10 bg-white/80 p-3">
                  <p className="text-xs uppercase tracking-[0.2em] text-ink/55">{t('intention.latestSeed')}</p>
                  <p className="mt-1 text-sm font-semibold text-ink/85">
                    {latestIntentionImprint?.certificateSeed ?? naLabel}
                  </p>
                </div>
                <div className="rounded-2xl border border-ink/10 bg-white/80 p-3">
                  <p className="text-xs uppercase tracking-[0.2em] text-ink/55">{t('intention.avgMappingConfidence')}</p>
                  <p className="mt-1 text-sm font-semibold text-ink/85">
                    {intentionAverageConfidence !== null ? formatPercent(intentionAverageConfidence) : naLabel}
                  </p>
                </div>
                <div className="rounded-2xl border border-ink/10 bg-white/80 p-3">
                  <p className="text-xs uppercase tracking-[0.2em] text-ink/55">{t('intention.confidenceTrend')}</p>
                  <p className="mt-1 text-sm font-semibold text-ink/85">
                    {intentionConfidenceDelta !== null ? formatSignedPercent(intentionConfidenceDelta) : naLabel}
                  </p>
                </div>
                <div className="rounded-2xl border border-ink/10 bg-white/80 p-3">
                  <p className="text-xs uppercase tracking-[0.2em] text-ink/55">{t('intention.avgRitualIntensity')}</p>
                  <p className="mt-1 text-sm font-semibold text-ink/85">
                    {intentionAverageRitualIntensity !== null ? formatPercent(intentionAverageRitualIntensity) : naLabel}
                  </p>
                  <p className="text-xs text-ink/55">{t('intention.topKeyword')}: {intentionTopKeyword ?? naLabel}</p>
                </div>
              </div>
              ...
              <div className="mt-4 grid gap-3">
                {intentionHistory.map((entry, index) => (
                  <div key={entry.id} className="rounded-2xl border border-ink/10 bg-white/80 p-3">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <p className="text-sm font-semibold text-ink/85">
                        {t('intention.imprintNumber', { number: intentionHistory.length - index })} · {formatDateTimeLabel(entry.createdAt)}
                      </p>
                      <span className="text-xs text-ink/60">
                        {t('intention.confidenceLabel')} {formatPercent(entry.mappingConfidence)} · {t('intention.intensityLabel')} {formatPercent(entry.ritualIntensity)}
                      </span>
                    </div>
                    <p className="mt-1 text-xs text-ink/60">
                      {entry.intentionText.length > 140
                        ? `${entry.intentionText.slice(0, 137)}...`
                        : entry.intentionText || t('intention.noIntentionTextRecorded')}
                    </p>
                    <p className="mt-1 text-xs text-ink/55">
                      {t('intention.keywordsLabel')}: {formatKeywordsLabel(entry.extractedKeywords)} · {t('intention.fieldLabel')}: {formatHzLabel(entry.mappedFrequencies)}
                    </p>
                    <p className="mt-1 text-xs text-ink/50">
                      {t('intention.modLabel')} {entry.modulationRateHz.toFixed(2)}Hz / {entry.modulationDepthHz.toFixed(2)} {t('intention.depthLabel')} · {t('intention.seedLabel')}{' '}
                      {entry.certificateSeed ?? t('naCompact')}
                    </p>
                  </div>
                ))}
              </div>
